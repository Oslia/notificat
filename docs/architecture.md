...
`NotificationService` は現在、最新メッセージと未読件数を集約する段階である。次の段階では通知履歴と既読操作を実装する。ネットワーク機能を追加する際は天気との同時 TLS 通信を避ける共通ジョブキューを導入する。UI はすでに `AppContext` を介してサービスを参照しており、新しい機能も同じ依存注入規則に従う。
